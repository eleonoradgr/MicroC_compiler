...
%inline init_var:
  ...
  | ASS e=ex 
      { Some([e]) }
  | ASS LBRACE l=separated_list(",", ex) RBRACE
      { Some(l) }
  | ASS s=STRING
      { Some( bulid_string s) }
;
...
stm:
  ...
  | WHILE LPAR e=ex RPAR s=stm
    { make_node $loc (While(e,s)) }
  | DO b=block WHILE LPAR e=ex RPAR ";"
...
ex:
  ...
  | id=ID LPAR l=separated_list("," ,ex ) RPAR
...
assop:
  ...
  | INC a=acc
    { let new_exp = make_node $loc (BinaryOp(Add, (make_node $loc (Access a)), (make_node $loc (ILiteral 1)))) in
      make_node $loc (Assign(a,new_exp)) }
  | a=acc INC
    { let new_exp = make_node $loc (UnaryOp(Inc, (make_node $loc (Access a)))) in
      make_node $loc (Assign(a,new_exp)) }
  | DEC a=acc
    {let new_exp = make_node $loc (BinaryOp(Sub, (make_node $loc (Access a)), (make_node $loc (ILiteral 1)))) in
      make_node $loc (Assign(a,new_exp))}
  | a=acc DEC
    {let new_exp = make_node $loc (UnaryOp(Dec, (make_node $loc (Access a)))) in
      make_node $loc (Assign(a,new_exp))}
;
